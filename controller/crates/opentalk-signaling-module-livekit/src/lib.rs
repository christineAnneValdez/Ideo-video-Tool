...
use opentalk_signaling_core::{
    CleanupScope, DestroyContext, Event, InitContext, ModuleContext, Participant, SignalingModule,
...
pub struct Livekit {
    ...
    participant_user_id: Option<String>,
    participant_oidc_sub: Option<String>,
    participant_first_name: Option<String>,
    participant_display_name: Option<String>,
...
impl SignalingModule for Livekit {
    ...
    async fn init(
        ...
        let (
            participant_user_id,
            participant_oidc_sub,
            participant_first_name,
            participant_display_name,
        ) = match ctx.participant() {
            Participant::User(user) => (
                Some(user.id.to_string()),
                Some(user.oidc_sub.clone()),
                Some(user.firstname.clone()),
                Some(user.display_name.to_string()),
            ),
            Participant::Guest | Participant::Sip | Participant::Recorder => (None, None, None, None),
        };
...
impl Livekit {
    ...
    async fn create_room_and_access_token(
        ...
        let token_name = self
            .participant_display_name
            .clone()
            .or_else(|| self.participant_first_name.clone())
            .unwrap_or_else(|| identity.clone());
        let mut attributes: Vec<(String, String)> = vec![(
            "participantId".to_string(),
            self.participant_id.to_string(),
        )];
        if let Some(user_id) = &self.participant_user_id {
            attributes.push(("userId".to_string(), user_id.clone()));
            attributes.push(("id".to_string(), user_id.clone()));
        }
        if let Some(oidc_sub) = &self.participant_oidc_sub {
            attributes.push(("sub".to_string(), oidc_sub.clone()));
            attributes.push(("oidc_sub".to_string(), oidc_sub.clone()));
        }
        if let Some(first_name) = &self.participant_first_name {
            attributes.push(("firstName".to_string(), first_name.clone()));
            attributes.push(("given_name".to_string(), first_name.clone()));
        }
        if let Some(display_name) = &self.participant_display_name {
            attributes.push(("displayName".to_string(), display_name.clone()));
            attributes.push(("name".to_string(), display_name.clone()));
            attributes.push(("preferred_username".to_string(), display_name.clone()));
        }
