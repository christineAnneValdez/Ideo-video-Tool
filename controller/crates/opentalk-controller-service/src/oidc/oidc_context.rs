...
use super::{
    IntrospectInfo, JWTAccessTokenClaims, JWTLogoutTokenClaims, JWTUserInfoFallbackClaims,
    OidcTokenHandler, OnlyExpiryClaim, OpenIdConnectUserInfo, OpenTalkAdditionalClaims,
    ProviderClient, RealmRoles, ServiceClaims, VerificationInfo, VerifyError, jwt,
};
use crate::Result;
...
impl OidcContext {
    ...
    async fn user_info(
        ...
        let access_token_secret = access_token.secret().to_string();
        let client = self
            .http_introspect_and_userinfo_client
            .as_ref()
            .unwrap_or(&self.http_client);
        let claims_result: Result<UserInfoClaims<OpenTalkAdditionalClaims, CoreGenderClaim>, _> = self
            .provider
            .client
        ...
        let claims: UserInfoClaims<OpenTalkAdditionalClaims, CoreGenderClaim> = match claims_result {
            Ok(claims) => claims,
            Err(err) => {
                log::warn!(
                    "Failed to fetch userinfo, attempting JWT fallback: {}",
                    Report::from_error(err)
                );

                if let Ok(jwt_claims) =
                    self.verify_jwt_token::<JWTUserInfoFallbackClaims>(&access_token_secret)
                {
                    let preferred_username = jwt_claims
                        .preferred_username
                        .clone()
                        .unwrap_or_else(|| "user".to_string());
                    let firstname = jwt_claims
                        .given_name
                        .clone()
                        .or_else(|| jwt_claims.name.clone())
                        .unwrap_or_else(|| preferred_username.clone());
                    let lastname = jwt_claims.family_name.clone().unwrap_or_else(|| "User".to_string());
                    let email = jwt_claims
                        .email
                        .clone()
                        .unwrap_or_else(|| format!("{preferred_username}@example.local"))
                        .to_lowercase();

                    return Ok(OpenIdConnectUserInfo {
                        sub: jwt_claims.sub,
                        email,
                        firstname,
                        lastname,
                        ...
                        locale: None,
                        timezone: None,
                        groups: jwt_claims.opentalk.x_grp,
                        phone_number: None,
                        display_name: jwt_claims.preferred_username.or(jwt_claims.name),
                        tenant_id: jwt_claims.opentalk.tenant_id,
                        tariff_id: jwt_claims.opentalk.tariff_id,
                        tariff_status: jwt_claims.opentalk.tariff_status,
                    });
                }

                return Err(CaptureApiError::from(
                    ApiError::internal().with_message("Failed to fetch userinfo"),
                ));
            }
        };
