...
/// Extended JWT access token claims used as a fallback when the OIDC userinfo endpoint
/// cannot be queried (e.g. tokens missing `openid` scope in local dev setups).
#[derive(Deserialize, Debug)]
pub struct JWTUserInfoFallbackClaims {
    /// Expires at
    #[serde(with = "time")]
    pub exp: DateTime<Utc>,
    /// Subject
    ...
    /// Optional OpenTalk specific additional claims
    #[serde(flatten)]
    pub opentalk: OpenTalkAdditionalClaims,
}

impl jwt::VerifyClaims for JWTUserInfoFallbackClaims {
    fn exp(&self) -> DateTime<Utc> {
        self.exp
    }
}
