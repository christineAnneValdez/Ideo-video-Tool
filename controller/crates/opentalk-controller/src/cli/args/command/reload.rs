...
/// Reloading via SIGHUP is not supported on Windows.
#[cfg(not(unix))]
fn send_sighup_to_processes(_processes: Vec<Pid>) -> Result<()> {
    println!("Reload command is not supported on Windows.");
    Ok(())
}
