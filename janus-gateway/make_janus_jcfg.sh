...
cat << EOF > /usr/local/etc/janus/janus.jcfg
...
general: {
	...
	admin_secret = "${JANUS_ADMIN_SECRET:-"janusoverlord"}"	# String that all Janus requests must contain
	...
	${JANUS_EVENT_LOOPS+"event_loops = $JANUS_EVENT_LOOPS"}
	...
	${JANUS_EVENT_LOOPS+"allow_loop_indication = true"}
...
media: {
	ipv6 = "${JANUS_ENABLE_IPV6:-"true"}"
	#ipv6_linklocal = true
	#min_nack_queue = 500
	rtp_port_range = "${JANUS_UDP_PORT_RANGE:-"20000-40000"}"
	#dtls_mtu = 1200
	#no_media_timer = 1
...
nat: {
	...
	ignore_mdns = ${JANUS_IGNORE_MDNS:-"false"}
	...
	ice_enforce_list = "${JANUS_ICE_IF:-"eth0"}"
...
plugins: {
	disable = "libjanus_voicemail.so,libjanus_recordplay.so,libjanus_streaming.so"
}
...
transports: {
	disable = "libjanus_nanomsg.so,${JANUS_DISABLE_WEBSOCKET+",libjanus_websockets.so"}${JANUS_DISABLE_HTTP+",libjanus_http.so"}${JANUS_DISABLE_RABBITMQ+",libjanus_mqtt.so"}"
}
